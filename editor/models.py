...
class NewQuestion(models.Model):
    ...
    def ready_to_publish(self, user):
        ...
        if self.editoritem.published == True:
            return True

        if not self.editoritem.can_be_edited_by(user):
            return False

        edit_content = self.editoritem.get_parsed_content().data
        metadata = edit_content.get('metadata',{})

        return (
            edit_content.get('statement','') 
            and edit_content.get('parts',[]) 
            and edit_content.get('variables',[]) 
            and edit_content.get('advice','') 
            and edit_content.get('name') 
            and self.editoritem.licence 
